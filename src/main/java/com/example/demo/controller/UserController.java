        ...
        try {
            ...
            if (user != null) {
                ...
                Map<String, Object> userData = Map.of(
                        ...
                        "memberSince",
                        user.getMemberSince() != null ? user.getMemberSince() : new java.util.Date().toString());
    ...
    public ResponseEntity<?> updateUser(@PathVariable String id, @RequestBody User user) {
        try {
            if (!id.equals(user.getId())) {
                return ResponseEntity.badRequest()
                        .body(Map.of("message", "User ID in path does not match user ID in body."));
            }
            // Before updating, fetch the existing user to preserve password and memberSince
            ...
            if (!existingUser.getEmail().equalsIgnoreCase(user.getEmail())) {
                if (userService.isEmailRegistered(user.getEmail())) {
                    return ResponseEntity.badRequest()
                            .body(Map.of("message", "Email address is already registered to another account."));
                }
            }
